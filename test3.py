...
# Loading credentials from config.properties
config = configparser.ConfigParser()
config.read("config.properties")

# Reading username and password from config.properties
...
def process_parquet_files(zip_path, json_dir, error_log_path):
    ...
    print(f"Manifest file saved in {manifest_path}")


# Calling the function process_parquet_files
process_parquet_files(zip_path, json_directory, error_log_path)


def zip_json_directory(json_dir):
    zip_file_path = f"{json_dir}.zip"  # Output zip file

    # Create the zip file
    shutil.make_archive(json_dir, 'zip', json_dir)
...
zip_json_directory(json_directory)
